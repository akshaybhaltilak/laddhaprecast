    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <a href="/" className="flex items-center space-x-4 group">
            ...
            <div className="flex flex-col">
              <span className={`text-xl font-bold tracking-tight transition-all duration-500 ${isScrolled
                  ? 'text-gray-900'
                  : 'text-white'
                }`}>
                Laddha Precast Industries
              </span>
              <span className={`text-xs transition-all duration-500 ${isScrolled
                  ? 'text-orange-600'
                  : 'text-orange-400'
                }`}>
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className={`relative px-5 py-2 rounded-lg font-semibold transition-all duration-300 group ${isScrolled
                    ? 'text-gray-700 hover:text-orange-600'
                    : 'text-gray-200 hover:text-orange-400'
                  }`}
              >
                <span className={`absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isScrolled ? 'bg-orange-50' : 'bg-white/5'
                  }`}></span>
                <span className="relative z-10">{link.name}</span>
                <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-0.5 transition-all duration-300 group-hover:w-3/4 ${isScrolled ? 'bg-orange-600' : 'bg-orange-400'
                  }`}></span>
              </a>
            ))}
            ...
            <a
              href="/contact"
              className={`ml-4 px-6 py-2.5 rounded-full font-bold transition-all duration-300 relative overflow-hidden group ${isScrolled
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-500 ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'
          ...
          <div className={`py-4 rounded-2xl my-4 transition-all duration-300 ${isScrolled
            ...
            <div className="flex flex-col space-y-1 px-2">
              {navLinks.map((link, index) => (
                <a
                  key={link.name}
                  href={link.href}
                  className={`py-3 px-6 rounded-xl font-semibold transition-all duration-300 transform ${isScrolled
              ...
              <a
                href="/contact"
                className={`mt-4 py-3 px-6 rounded-xl font-bold text-center transition-all duration-300 ${isScrolled
